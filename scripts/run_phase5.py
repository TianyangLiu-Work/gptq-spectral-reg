#!/usr/bin/env python3
"""Phase 5: OPT-6.7B with group_size=32 and expanded beta sweep.
Tests whether spectral regularization needs smaller groups to be effective."""
...
# Sweep group sizes and betas
CONFIGS = [
    # group_size, label, method, beta
    (32, "FP16", None, None),
    (32, "GPTQ_base", "none", 0.01),
    (32, "Damp_0.05", "stronger_damping", 0.05),
    (32, "Frob_0.001", "frobenius", 0.001),
    (32, "Frob_0.01", "frobenius", 0.01),
    (32, "Spectral_0.001", "spectral", 0.001),
    (32, "Spectral_0.003", "spectral", 0.003),
    (32, "Spectral_0.01", "spectral", 0.01),
    (128, "Spectral_0.0003", "spectral", 0.0003),
    (128, "Spectral_0.001", "spectral", 0.001),
    (128, "Spectral_0.003", "spectral", 0.003),
]
...
def main():
    ...
    # Spectral estimation (based on g128 GPTQ_base quantization)
    ...
    for gs, label, method, beta in CONFIGS:
        if label == "FP16":
            continue
        print(f"\n{'='*70}", flush=True)
        print(f"  {label} gs={gs} (method={method}, beta={beta})", flush=True)
        ...
        for n in quant_names:
            ...
            # Re-quantize to target group_size
            q_w_g = quantize_weight_groupwise(q_w.cpu(), bits=BITS, group_size=gs)[0]
            deltas[n] = q_w_g.cpu() - refs[n]
            del H, w, q_w, q_w_g
        ...
        results.append({"method": label, "group_size": gs, "PPL": ppl,
                        "E_calib": ec, "E_test": et, "Spec": sp, "Time": t_q})
        del deltas

    # Summary
    print(f"\n{'='*90}", flush=True)
    print(f"{'Method':20s} {'GS':5s} {'PPL':12s} {'E_calib':12s} {'E_test':12s} {'Spec':12s} {'Time':8s}", flush=True)
    print(f"{'-'*90}", flush=True)
    for r in results:
        gs = r.get("group_size", "")
        print(f"{r['method']:20s} {str(gs):5s} {r['PPL']:12.4f} {r['E_calib']:12.6f} {r['E_test']:12.6f} {r['Spec']:12.6f} {r['Time']:8.1f}", flush=True)

    Path("experiments/phase5").mkdir(parents=True, exist_ok=True)
    with open("experiments/phase5/summary.json", "w") as f:
        json.dump({"model": MODEL_NAME, "bits": BITS, "calib_samples": CALIB_SAMPLES,
                   "configs": [{"method": m, "group_size": g, "beta": b}
                               for g, _, m, b in CONFIGS if m is not None],
                   "results": results}, f, indent=2)
    print(f"\nSaved to experiments/phase5/summary.json", flush=True)
